...
const Hyperbot = observer(() => {
                        ...
                        if (enableEntryPointRef.current) {
                            ...
                            if (typeof digit === "number" && digit === expectedDigit) {
                                if (!entryPointMet.current) {
                                    ...
                                    console.log(`✅✅✅ Entry point digit ${digit} matched! Expected: ${expectedDigit}. Trading can proceed now! ✅✅✅`);
                                }
                            } else {
                                // Debug: log when digit doesn't match (only occasionally to avoid spam)
                                if (Math.random() < 0.001 && !entryPointMet.current) {
                                    console.log(`🔍 Entry point check - Received digit: ${digit} (type: ${typeof digit}), Expected: ${expectedDigit} (type: ${typeof expectedDigit})`);
                                }
                            }
                        }
    ...
    const startAutoTrading = async () => {
        ...
        if (enableEntryPoint) {
            ...
            console.log(`📊 Entry point refs - enabled: ${enableEntryPointRef.current}, digit: ${entryPointDigitRef.current}`);
        } else {
        ...
        if (enableEntryPointRef.current) {
            console.log(`🔄 Entry point reset. Waiting for digit: ${entryPointDigitRef.current} to appear in tick stream...`);
        }
        ...
        const tradeInterval = async () => {
            ...
            if (enableEntryPointRef.current) {
                ...
                if (!entryPointMet.current) {
                    ...
                    console.log(`✅ Entry point condition met! Expected digit ${entryDigit} was detected. Proceeding with trades...`);
                }
            }
            ...
            try {
                ...
                if (lockEntryPoint && enableEntryPointRef.current) {
                    entryPointMet.current = false;
                    console.log(`🔒 Entry point locked. Waiting for digit ${entryPointDigitRef.current} before next trade...`);
                }
            } catch (error) {
                                ...
                                onChange={(e) => {
                                    ...
                                    if (value === "") {
                                        ...
                                        if (!isNaN(num) && num >= 0 && num <= 9) {
                                            ...
                                            console.log(`📝 Entry point digit changed to ${num} (type: ${typeof num}). Waiting for this digit...`);
                                        }
                                    }
                                }}
                                onBlur={(e) => {
